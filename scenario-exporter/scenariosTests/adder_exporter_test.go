...
import (
	...
	mge "github.com/Dharitri-org/drtg-scenario/scenario-exporter"

	mj "github.com/Dharitri-org/drtg-scenario/model"
	"github.com/Dharitri-org/drtg-scenario/util"
...
func TestGetAccountsAndTransactionsFrom_Adder(t *testing.T) {
	...
	scAccount := mge.SetNewAccount(0, append(mge.ScAddressPrefix, addressAdder[mge.ScAddressPrefixLength:]...), big.NewInt(0), make(map[string][]byte), util.GetSCCode("adder.wasm"), addressOwner)
	...
	transaction := mge.CreateTransaction("add", [][]byte{{3}}, 0, big.NewInt(0), make([]*mj.DCTTxData, 0), sbi.Accs[0].GetAddress(), sbi.Accs[1].GetAddress(), 5000000, 1)
...
func TestGetAccountsAndTransactionsFrom_AdderWithExternalSteps(t *testing.T) {
	...
	transactionAlice := mge.CreateTransaction("add", [][]byte{{3}}, 0, big.NewInt(0), make([]*mj.DCTTxData, 0), sbi.Accs[0].GetAddress(), sbi.Accs[1].GetAddress(), 5000000, 1)
	transactionBob := mge.CreateTransaction("add", [][]byte{{3}}, 0, big.NewInt(0), make([]*mj.DCTTxData, 0), sbi.Accs[2].GetAddress(), sbi.Accs[1].GetAddress(), 5000000, 1)
	transactionOwner := mge.CreateTransaction("add", [][]byte{{3}}, 0, big.NewInt(0), make([]*mj.DCTTxData, 0), sbi.Accs[3].GetAddress(), sbi.Accs[1].GetAddress(), 5000000, 1)
